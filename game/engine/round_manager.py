...
class RoundManager:
    ...
    @classmethod
    def serialize(self, string1, string2, boolean):
        return f"{string1}|{string2}|{int(boolean)}"
    ...
    def __showdown(self, state):
        ...
        for player in state["table"].seats.players:
            # print(
            #    f"Player {player.name}'s hole cards: "
            #    + str(player.hole_card[0])
            #    + str(player.hole_card[1])
            # )
            res.append(str(player.hole_card[0]) + str(player.hole_card[1]))
        iwin = winners[0].name == "me"
        # print(f"Win? {iwin}")
        res.append(iwin)
        self.results.append(res)
        if len(self.results) == 100:
            with open(f"results/{os.getpid()}.txt", "a") as f:
                for res in self.results:
                    f.write(self.serialize(res[0], res[1], res[2]) + "\n")
                self.results.clear()
