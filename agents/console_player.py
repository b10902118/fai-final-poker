...
threat_map = [0, 0, 0.5, 1]


def tmap(num):
    if num >= len(threat_map):
        return threat_map[-1] + num
    elif num < 0:
        return 0
    return threat_map[int(num)] + num
...
#            print(f"{ischeck=}")
        ...
        threshold_eq = 0.45 * threat_rate + min(0.04 * tmap(self.op_rc_cnt), 0.15)

        orig_pot_odds = c / (pot + c)
        pot_odds = c / (pot + c) + min(
            0.03 * tmap(self.op_rc_cnt) * (threat_rate * 2), 0.12
        )
#        print(f"{self.hole_card} {community_card}")
#        print(f"{self.op_target=} {ca+self.pre_bet=}")
#        print(f"{pot_odds=} {orig_pot_odds=}")
#        print(f"{eq=} {threat_rate=} {threshold_eq=}")
        # print(valid_actions)
...
#                print(f"eq < 0.6, rand_act {action=} {amount=}")
            else:  # >= 0.6
                action, amount = (
                    bait_raise() if ischeck or self.opening else agg_raise()
                )
#                print(
#                   f"eq > 0.6, {'bait' if ischeck or self.opening else 'agg'} {action=} {amount=}"
#               )
