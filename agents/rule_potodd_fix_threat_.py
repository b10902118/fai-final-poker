...
class ConsolePlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        threshold_eq = 0.50 * threat_rate + min(0.04 * self.op_rc_cnt, 0.15)

        pot_odds = c / (pot + c) + min(0.015 * self.op_rc_cnt, 0.08)
        print(f"{self.hole_card} {community_card}")
        print(f"{self.op_target=} {ca+self.pre_bet=}")
        print(f"{eq=} {baseline_eq=} {threat_rate=} {threshold_eq=}")
        # print(valid_actions)
